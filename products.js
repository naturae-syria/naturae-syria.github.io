const products = [
  ...
  category: "عناية بالشعر",
  description: `الوصف:
التركيز على الاحتفاظ بالماء داخل الشعرة بفضل البانثينول والألوفيرا.

لمن يُنصح به؟
الشعر الجاف/الباهت أو المُجهَد يوميًا بالحرارة والتصفيف.
(خط المنتج موجّه لترطيب واسترجاع الحيوية).`,
...
عدد المرات:
لا تُحدِّد ناتورا عددًا ثابتًا؛ قوليًا: استخدميه بانتظام حسب حاجة شعرك.
إن كنتِ تتبعين “الجدول الزمني للعناية بالشعر (Cronograma Capilar)” يوضع هذا الماسك في أيام الترطيب ضمن الجدول.

مكوّنات محورية ولماذا تفيد:
- Panthenol (Pro-Vitamin B5): يجذب الرطوبة ويقلّل الجفاف، فيترك الشعرة أنعم وأكثر مرونة.
- Aloe Vera: يساعد على ترطيب الألياف وتهدئة فروة الرأس، مع تأثير لطيف غير مُثقِّل.

نتائج متوقعة:
شعر أنعم، لامع، وأكثر سلاسة من أول استعمال، مع عطر فاكهي منعش (تفاح أخضر).`,

  explanation: `الشرح:
نصائح عملية (مفيدة وغير مخالفة لتوجيهات الشركة):
- في يوم استخدام الماسك، يمكن الاستغناء عن البلسم لأن الماسك يقوم بدوره بتركيز أعلى.
- إن كان شعرك شديد الجفاف، زيدي تكرار الاستخدام ضمن أسبوعك، مع الالتزام بزمن الـ 3 دقائق فقط لتجنّب الثِّقل.`,
...
},
{
  ...
  descriptionBlocks: [
    "التركيز على الاحتفاظ بالماء داخل الشعرة بفضل البانثينول والألوفيرا.",
    "لمن يُنصح به؟ الشعر الجاف/الباهت أو المُجهَد يوميًا بالحرارة والتصفيف. (خط المنتج موجّه لترطيب واسترجاع الحيوية)."
  ],

  usageSteps: [
    "اغسلي الشعر واتركيه رطبًا (غير مبلل تمامًا).",
    "خذي كمية مناسبة ووزّعيها من منتصف الطول حتى الأطراف، مع تجنب الجذور.",
    "اتركيه 3 دقائق بالضبط.",
    "اشطفي جيدًا."
  ],
  usageFrequency: "لا تُحدِّد ناتورا عددًا ثابتًا؛ يُستخدم بانتظام حسب حاجة الشعر. ضمن Cronograma Capilar يوضع في أيام الترطيب.",

  ingredients: [
    { name: "Panthenol (Pro-Vitamin B5)", benefit: "يجذب الرطوبة ويقلّل الجفاف؛ نعومة ومرونة أعلى." },
    { name: "Aloe Vera", benefit: "ترطيب الألياف وتهدئة الفروة بتأثير لطيف غير مُثقِّل." }
  ],

  results: [
    "شعر أنعم، لامع، وأكثر سلاسة من أول استعمال.",
    "عطر فاكهي منعش (تفاح أخضر)."
  ],

  tips: [
    "في يوم استخدام الماسك يمكن الاستغناء عن البلسم لأن الماسك يقوم بدوره بتركيز أعلى.",
    "للشعر شديد الجفاف: زيدي التكرار مع الالتزام بزمن 3 دقائق لتجنّب الثِّقل."
  ],
...
// دوال تنسيق ترجع نصوص مرتّبة بس باستخدام \n (بدون CSS)
export function formatDescription(p = product) {
  return p.descriptionBlocks.join("\n\n");
}

export function formatUsage(p = product) {
  const steps = p.usageSteps
    .map((s, i) => `${i + 1}) ${s}`)
    .join("\n");
  const ing = p.ingredients
    .map(x => `- ${x.name}: ${x.benefit}`)
    .join("\n");
  const res = p.results
    .map(x => `- ${x}`)
    .join("\n");
  ...
  return [
    "طريقة الاستخدام (رسمية خطوة بخطوة):",
    steps,
    "",
    "عدد المرات:",
    p.usageFrequency,
...
export function formatExplanation(p = product) {
  const tips = p.tips.map(t => `- ${t}`).join("\n");
  return ["نصائح عملية:", tips].join("\n");
}