...
If requests to the WhatsApp API fail with `401 Unauthorized`, verify all
credentials in the Meta dashboard:
...
Use `pnpm run check-wa-token` to confirm that the access token is valid for your
app. The command calls the Graph API `/debug_token` endpoint and prints the
token details.
