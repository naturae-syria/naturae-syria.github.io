    ...
    for (const change of changes) {
      ...
      for (const msg of messages) {
        ...
        try {
          ...
          if (err.response) {
            console.error('WhatsApp API response', err.response.status, err.response.data);
            if (err.response.status === 401) {
              console.error(
                'Unauthorized - verify that WHATSAPP_TOKEN matches your app, the phone number ID belongs to your WhatsApp Business Account, and the token was generated with the correct App ID and App Secret.'
              );
            }
          }
        }
